...
function Navbar() {
    ...
    return (
        <>
              <nav className='bg-black lg:h-[25vh] h-[18vh] text-white flex justify-between px-8 md:px-16 lg:px-52 items-center w-auto overflow-hidden relative'>
      <img src="https://web-images.credcdn.in/_next/assets/images/home-page/cred-logo.png" width={40} height={45} alt="" />

      <div className='lg:hidden flex items-center'>
        <button className='text-white text-4xl lg:text-4xl md:text-5xl focus:outline-none' onClick={toggleMenu}>
      ...
      <div className={`lg:hidden fixed top-0 right-0 bg-[#0F0F0F] w-[375px] md:w-[400px] z-50 h-full transition-transform transform ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <button className='text-white text-4xl focus:outline-none md:ml-[295px] ml-[300px] md:mt-[73px] mt-10' onClick={toggleMenu}><RiCloseLine /></button>
          ...
          <a href="" className='text-white font-semibold text-sm'>credit score check</a>
            ...
            <div className='bg-black flex justify-center items-center z-10 flex-col text-[#F8F8F8] h-[75vh] w-full'>
                ...
                className='font-bold text-start lg:text-center lg:mt-[-80px] lg:w-[80vw] w-[80vw] text-5xl md:text-7xl lg:text-8xl'>rewards for paying <br />
                    ...
                    className='font-bold text-xs lg:mt-9 md:text-xl lg:text-xl'>join 9M+ members who win rewards and cashbacks everyday</motion.p>
                     </div>
                    ...
                    transition={{duration: 1}}
                     className='lg:px-10 px-5 hover:bg-transparent border-2 hover:text-white py-2 lg:py-3 mt-8 lg:mt-14 md:ml-12 lg:ml-0 md:mt-16 bg-white text-black font-semibold text-lg lg:text-xl rounded-full'>Download CRED</motion.button>
                    </div>
            </div>
        </>
    )
